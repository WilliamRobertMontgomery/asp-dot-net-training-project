...
namespace EnumerableTask
{
    ...
    public class Task
    {
        ...
        {
            return data.Select(x => (x ?? string.Empty).Length);
        }
        ...
        {
            long temp = 0;
            return data.Select(x => temp += x);
            // or
            // return data.Select((x, index) => (long)x + data.Take(index).Sum());
        }
        ...
        {
            return data.DefaultIfEmpty(String.Empty).Where(x => (x != null) && x.StartsWith(prefix, true, null));
        }
        ...
        {
            return data.DefaultIfEmpty(String.Empty).Aggregate<IEnumerable<char>>((result, current) => result.Union(current ?? String.Empty));
        }
        ...
        {
            return data.FirstOrDefault(x => (x ?? string.Empty).IndexOf("first", StringComparison.OrdinalIgnoreCase) >= 0);
        }
        ...
        {
            return data.Where(x => (x ?? string.Empty).Length == 3).Distinct().Count();
        }
        ...
        {
            return data.Where(x => (x ?? string.Empty).Length == data.Max(y => (y ?? string.Empty).Length)).Count();
        }
        ...
        {
            return "0123456789".Except(data.DefaultIfEmpty("").Aggregate((result, current) => result + current));
        }
        ...
        {
            return numbers.Zip(fruits, (number, fruit) => number + " " + fruit);
        }
        ...
        {
            return data.DefaultIfEmpty(String.Empty).Aggregate<IEnumerable<char>>((result, current) => result.Intersect(current)).OrderBy(x => x);
        }
        ...
        {
            return data.Select(x => (x ?? string.Empty).Length).Sum();
        }
        ...
        {
            return data.DefaultIfEmpty().All(x => (!string.IsNullOrEmpty(x)) && (x == x.ToUpper()));
        }
